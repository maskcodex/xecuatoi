 ...
 * */
    class STCart
    {
        ...
        static function init()
        {
            ...
            add_action('st_email_after_booking',array(__CLASS__,'send_mail_after_booking'));
...
// Reset content-type to avoid conflicts -- http://core.trac.wordpress.org/ticket/23578
        ...
        static function add_cart($item_id,$number=1,$price=false,$data=array())
        {
            ...
            if($st_is_woocommerce_checkout)
            {
                ...
                $price=self::get_item_total($cart_data,$item_id);
                $cart_data['price']=$price;

                $cart_data=apply_filters('st_add_to_cart_item_'.$data['st_booking_post_type'],$cart_data,$item_id);
                $product_id= self::_create_new_product($item_id,$cart_data);
        ...
        static function _create_new_product($item_id,$cart_item)
        {
            ...
            $total_cart_item_price=$cart_item['price'] * $cart_item['number'];
            ...
            {
                STTemplate::set_message(__('Sorry! Can not create product'));
                return false;
            }
            ...
            update_post_meta( $product_id, '_regular_price', $total_cart_item_price);
...
//
//                    update_post_meta( get_the_ID(), '_regular_price', $total_cart_item_price);
//                    update_post_meta( get_the_ID(), '_price', $total_cart_item_price);
//                    return get_the_ID();
//                }
//                wp_reset_postdata();
//            }else{
                ...
                foreach ($cart as $key => $value) {
                    ...
                    switch ($post_type) {
                        ...
                        case "st_rental":
                            ...
                            break;
                        case "st_cars":
                            $total +=STCars::get_cart_item_total($key,$value);
                            break;
                        case "st_tours":
                            $total +=STTour::get_cart_item_total($key,$value);
                            break;
                        case "st_activity":
                            $total +=STActivity::get_cart_item_total($key,$value);
                           // $total += $value['price'] * $value['number'];
                            break;
                    }
                }
