<?php
    ...
    if(!class_exists('STAdminUpdateContent')) {
        ...
        {
            function _init(){
                add_action('admin_menu', array(__CLASS__,'st_update_content_admin_menu'),50);
                add_action('admin_enqueue_scripts',array($this,'_add_style_javascript'));


                add_action( 'wp_ajax_st_my_update_content', array(__CLASS__,'st_my_update_content_func') );
                add_action( 'wp_ajax_nopriv_st_my_update_content', array(__CLASS__,'st_my_update_content_func') );
            }
            function _add_style_javascript()
            {
                wp_enqueue_script('update-content.js', get_template_directory_uri() . '/js/admin/update-content.js', array('jquery'), null, true);
            }
            ...
            static function st_control_update_content() {
                $data_json = array('st_hotel','st_rental','st_cars','st_tours','st_activity','hotel_room','location');
